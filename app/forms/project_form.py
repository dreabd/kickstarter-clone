...
from wtforms import StringField, TextAreaField, SelectField, IntegerField, DateField, TextAreaField
...
def date_checker(form, field):
    ...
    if form_date < date.today():
        raise ValidationError(
            "Please provide a valid date that is after today's date.")


class ProjectForm(FlaskForm):
    project_name = StringField('Project Name', validators=[DataRequired()])
    description = TextAreaField('Project Name', validators=[DataRequired(), Length(
        min=25, max=255, message="Description must be between 25 to 255 characters.")])
    ...
    state = StringField("State", validators=[DataRequired(), Length(
        min=2, max=2, message="Please use the state's two character abbreviation.")])
    story = TextAreaField("Story", validators=[DataRequired(), Length(
        min=200, message="Story must be at least 200 characters.")])
